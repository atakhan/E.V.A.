...
Сейчас исполнение одной работы (Skill → Action → Tool, FSM) подходит. Ломается слой **над** скиллом: ждущий run забирает всю сессию, фраза и жест — два агента, стол агент не читает.
...
- Конструктор FSM (`states[]` как канон скилла)
